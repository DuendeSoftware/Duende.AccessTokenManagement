...
public class ClientAssertionService : IClientAssertionService
{
    ...
    public ClientAssertionService(IOptionsSnapshot<ClientCredentialsClient> options, DiscoveryCaches discos)
    {
        _options = options;
        _discos = discos;
    }
    ...
    public async Task<ClientAssertion?> GetClientAssertionAsync(string? clientName = null, TokenRequestParameters? parameters = null)
    {
        if (clientName == "demo.jwt")
        {
            ...
            var discoCache = _discos.Get(clientName);

            var audience = discoCache == null ? options.TokenEndpoint 
                : (await discoCache.GetAsync()).TokenEndpoint;
